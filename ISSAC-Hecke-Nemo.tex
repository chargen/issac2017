...
reduction modulo $p$ of the minimal polynomial $m(T)$ of $M$ if $p$ doesn't
divide the discriminant of $m(T)$. Thus, we may have to exclude finitely
many primes $p$ from the Chinese remaindering. This is detected if the
degrees of the minimal polynomials modulo $p$ rise at any point during the
algorithm.
...
Namely, for one of the (nondegenerate) primes $p$, we record which standard
