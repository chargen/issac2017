...
typically extremely pessimistic. We require a much better termination
criterion that guarantees correctness. 

It is unfortunately too expensive to simply evaluate the minimal polynomial
...
the minimum polynomial, giving us our termination condition.
...
\item Let $\gamma = 1/\alpha$, $\langle g\rangle = \alpha \mathcal O_K \cap \ZZ$,
 $g = g_1g_2$ with coprime $g_1$, $g_2$ and $g_2$ only divisible be primes not in $S$. Then $A^{-1} = \langle 1, g_2\gamma\rangle$ is an $S$-normal presentation.
...
Hecke and Nemo were developed during the DFG priority project SPP 1489.
The last author was partially supported by ERC Starting Grant ANTICS 278537.
